...
// // Bỏ dòng import sai này: import { LarkService } from './services/larkService';
...
//         console.log(`[DEBUG] Uploading document: ${fileName}`);
//         console.log(`[DEBUG] File size: ${documentBuffer.length} bytes`);
//         console.log(`[DEBUG] MIME type: ${FileHelper.getMimeType(fileName)}`);
//         console.log(`[DEBUG] Using type: "attachment"`);
...
//         console.log(`[SUCCESS] Upload thành công: ${fileName}, Code: ${result.data.code}`);
//         return result.data.code;
//     }
...
export class LarkService {
    ...
    // Thêm method để lấy approval definition
    async getApprovalDefinition(approvalCode: string): Promise<any> {
        const url = `https://open.larksuite.com/open-apis/approval/v4/approvals/${approvalCode}`;
        
        console.log(`[DEBUG] Getting approval definition for code: ${approvalCode}`);
        ...
        console.log(`[DEBUG] Approval definition response:`, JSON.stringify(data, null, 2));
        
        if (data.code !== 0) {
            throw new Error(`Lỗi lấy approval definition: ${data.msg || data.message}`);
        }
